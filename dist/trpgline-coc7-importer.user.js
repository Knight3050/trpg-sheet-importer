  ...
  `;
    ...
    async function importEntries(group, entries, report) {
        for (const [name, value] of entries) {
            ...
            if (!row) {
                report.push(
                    createReportEntry(group, name, value, { ok: false, reason: '未找到字段' })
                );
                continue;
            }
    ...
    async function importParagraphEntries(group, entries, report) {
        for (const [name, value] of entries) {
            ...
            if (!row) {
                report.push(
                    createReportEntry(group, name, value, { ok: false, reason: '未找到段落字段' })
                );
                continue;
            }
    ...
    function getActivePropertyTab(root) {
        ...
        const activeButton = [
            ...root.querySelectorAll(
                '.sheet-content--property .btn-primary, .sheet-content--property button'
            ),
    ...
    async function importWeapons(weapons, report) {
        ...
        if (rows.length === 0) {
            for (const weapon of weapons) {
                report.push(
                    createReportEntry('weapons', weapon.name, formatWeaponValue(weapon), {
                        ok: false,
                        reason: '未找到武器表',
                    })
                );
        ...
        for (const [index, weapon] of weapons.entries()) {
            ...
            report.push(
                createReportEntry('weapons', weapon.name, formatWeaponValue(weapon), result)
            );
        }
    }
    ...
    function mapWeaponFromColumns(cells, layout, row) {
        const name =
            String(cells[`${layout.name}${row}`] ?? '').trim() ||
        ...
        return (
            rows.find((row) => {
                ...
                return (
                    rowKey === wantedKey ||
                    rowKey === fallbackKey ||
                    rowKey === getMaxBaseKey(wantedKey)
                );
            }) ?? null
        );
    }
    ...
    function findNumberInput(row) {
        return (
            row.querySelector('.character-attribute__value input[type="number"]') ??
            row.ownerDocument?.querySelector(
                '.character-attribute__value input[type="number"], input[type="number"]'
            ) ??
            null
        );
    }
    ...
    function isTextAttributeRow(row) {
        return row.classList?.contains('character-attribute--text') || Boolean(findTextInput(row));
    }

    function isWeaponHeaderRow(row) {
        const labels = [...(row.querySelectorAll?.('.character-attribute--row--column') ?? [])].map(
            (column) => normalizeAlias(column.textContent)
        );
    ...
    function findTextInput(column) {
        return (
            column.querySelector('textarea, input[type="text"], input:not([type])') ??
            column.ownerDocument?.querySelector(
