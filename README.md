...
| 技能表 | 技能值，包括带分支的技能 |
| 战斗 | 武器名称、命中率、伤害和射程 |
| 身世背景 | 特质、思想及信念、重要之人等背景资料 |
| 物品 | 拥有的物品与资产、携带物品 |
| 物件資訊 | 公开资讯 |

角色卡的各项内容分布在不同选项卡中。请先切换到对应选项卡，再点击 `导入当前页`；每个选项卡需要分别执行一次导入。
...
4. 根据面板中的“可自动赋值的选项卡”清单切换到目标选项卡。
5. 确认预览信息后，点击 `导入当前页`，并对需要填写的选项卡逐页执行导入。
6. 根据导入报告检查是否有字段未找到或导入失败。

如果某些字段未导入成功，请确认当前打开的是对应选项卡，然后再次导入。
